        ...
        while True:
            if not self.sleep:
                ...
                if config.ENABLE_OFFLOADING:
                    # When offloading is enabled, choose the nearest drone with sufficient capacity
                    nearest_drone = None
                    min_distance = float('inf')

                    for drone in drone_candidates:
                        distance = euclidean_distance(self.coords, drone.coords)
                        # Check if drone has processing capacity (if it's a UAV)
                        if hasattr(drone,
                                   'processing_capacity'):
                    ...
                    destination = nearest_drone if nearest_drone else random.choice(drone_candidates)
                else:
    ...
    def feed_packet(self):
        ...
        while True:
            if not self.sleep:
                ...
                current_second =  int(current_time // 1e6) * 1e6

                # Only record if we've entered a new second
                if current_second != last_second:
                    total_queue_length = sum(q.qsize() for q in self.priority_queues.values())

                    # Remove any previous entry for this second (if exists)
                    self.simulator.metrics.queue_length_history = [
                        entry for entry in self.simulator.metrics.queue_length_history
                        if (int(entry[0] // 1e6) * 1e6) != current_second
                    ]

                    # Add the new entry
                    self.simulator.metrics.queue_length_history.append(
                        (current_time, total_queue_length)
                    )
                ...
                if not self.blocking() and not Node.transmitting_queue.empty():
                    ...
                    if isinstance(packet, DataPacket):
                        ...
                        delay = (self.env.now - packet.creation_time) / 1e6  # delay in milliseconds
                        self.simulator.metrics.priority_delays[packet.priority].append(delay)
                        ...
                        if hasattr(self, 'processing_capacity'):
                            ...
                            # Check if we can process this packet locally
                            can_process_locally = required_rate <= self.processing_capacity
                            # OFFLOADING Or DROPPING
                            if config.ENABLE_OFFLOADING:
                                ...
                                if not can_process_locally:
                                    logging.warning(
                                        '[DROP] Packet %s dropped at Node %s - insufficient capacity (%.2f > %.2f MB/s)',
                                        packet.packet_id, self.identifier, required_rate, self.processing_capacity)
                                    self.simulator.metrics.packets_dropped_due_to_capacity += 1
                                    continue

                            # If we get here, the task can be processed locally
                            # Record the wait time
                            wait_time = self.env.now - packet.creation_time

                            # Calculate processing time
                            processing_time = (packet.size_mb / self.processing_capacity) * 1e6  # in ns

                            # Record metrics for locally processed task
                            self.simulator.metrics.locally_processed_tasks += 1
                            self.simulator.metrics.task_wait_times.append((wait_time, processing_time))
                            self.simulator.metrics.completion_times[
                                packet.packet_id] = self.env.now + processing_time

                            yield self.env.timeout(processing_time)

                            logging.info(
                                f'[PROCESSED] Node {self.identifier} completed task {packet.packet_id} '
                                f'after {processing_time / 1e6:.2f}ms processing')
                            continue

                        # Routing logic for packets that don't require processing
